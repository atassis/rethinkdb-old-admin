...
Handlebars.registerHelper('comma_separated', function(context, block) {
    let out = "";
    for (let i = 0, end = context.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
        out += block(context[i]);
        if (i !== (context.length-1)) { out += ", "; }
    }
    return out;
});
...
Handlebars.registerHelper('links_to_servers', function(servers, safety) {
    let out = "";
    for (let i = 0, end = servers.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
        if (servers[i].exists) {
            out += '<a href="#servers/'+servers[i].id+'" class="links_to_other_view">'+servers[i].name+'</a>';
        } else {
            out += servers[i].name;
        }
        if (i !== (servers.length-1)) { out += ", "; }
    }
...
Handlebars.registerHelper('links_to_datacenters_inline_for_replica', function(datacenters) {
    let out = "";
    for (let i = 0, end = datacenters.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
        out += '<strong>'+datacenters[i].name+'</strong>';
        if (i !== (datacenters.length-1)) { out += ", "; }
    }
    return new Handlebars.SafeString(out);
});

// Helpers for pluralization of nouns and verbs
const pluralize_noun = function(noun, num, capitalize) {
    let result;
    if (noun == null) { return 'NOUN_NULL'; }
...
Handlebars.registerHelper('pluralize_verb_to_have', function(num) { if (num === 1) { return 'has'; } else { return 'have'; } });
const pluralize_verb = function(verb, num) { if (num === 1) { return `${verb}s`; } else { return verb; } };
...
const humanize_uuid = function(str) { if (str != null) { return str.substr(0, 6); } else { return "NULL"; } };
...
const humanize_table_status = function(status) {
    if (!status) {
        return "";
...
const approximate_count = function(num) {
    // 0 => 0
    // 1 - 5 => 5
    // 5 - 10 => 10
    // 11 - 99 => Rounded to _0
    ...
    if (num === 0) {
        ...
        if (approx < 100) { // We just want one digit
            ...
            if (result.length === 1) { // In case we have 4 for 4000, we want 4.0
                result = result + '.0';
            }
            return result+'K';
...
const format_bytes = function(bytes, decimals) {
...
Handlebars.registerHelper('if_defined', function(condition, options) {
    if (typeof condition !== 'undefined') { return options.fn(this); } else { return options.inverse(this); }
});

// Extract form data as an object
const form_data_as_object = function(form) {
...
var deep_copy = function(data) {
    let result, value;
    if ((typeof data === 'boolean') || (typeof data === 'number') || (typeof data === 'string') || (typeof data === 'number') || (data === null) || (data === undefined)) {
        return data;
...
const date_to_string = function(date) {
    ...
    if (d.getTimezoneOffset() !== d_shifted.getTimezoneOffset()) {
        ...
        const d_shifted_bis = new Date(((date.epoch_time+timezone_int)-((d.getTimezoneOffset()-d_shifted.getTimezoneOffset())*60))*1000);
        ...
        if (d_shifted.getTimezoneOffset() !== d_shifted_bis.getTimezoneOffset()) {
            ...
            const str_pieces = d_shifted_bis.toString().match(/([^ ]* )([^ ]* )([^ ]* )([^ ]* )(\d{2})(.*)/);
...
const prettify_duration = function(duration) {
    if (duration === null) {
        return '';
    } else if (duration < 1) {
        return '<1ms';
...
const binary_to_string = function(bin) {
    ...
    let number_of_equals, sizeStr;
    const blocks_of_76 = Math.floor(bin.data.length/78); // 78 to count \r\n
    const leftover = bin.data.length-(blocks_of_76*78);

    const base64_digits = (76*blocks_of_76)+leftover;

    const blocks_of_4 = Math.floor(base64_digits/4);

    const end = bin.data.slice(-2);
    if (end === '==') {
        number_of_equals = 2;
    } else if (end.slice(-1) === '=') {
        number_of_equals = 1;
    ...
    if (size === 0) {
        ...
        const str = atob(bin.data.slice(0, 8));
        let snippet = '';
...
const replica_rolename = function({configured_primary: configured, 
...
const replica_roleclass = function({configured_primary: configured, 
                      currently_primary: currently}) {
    if (configured && currently) {
        return "primary";
    } else if (configured && !currently) {
...
const state_color = function(state) {
  switch (state) {
