...
class Driver {
    static initClass() {
        ...
        this.prototype.queries = {
            all_logs: limit => {
                const server_conf = driver.admin().server_config;
            ...
            tables_with_primaries_not_ready(
                ...
                table_status) {
                    ...
                    return r.do(driver.admin().server_config
                            .map(x => [x('id'), x('name')]).coerceTo('ARRAY').coerceTo('OBJECT'),
                        server_names => table_status.map(table_config_id, (status, config) => ({
                            id: status('id'),
                            name: status('name'),
                            db: status('db'),
                            ...
                            }).filter(shard => shard('primary_state').ne('ready')).coerceTo('array')
            ...
            tables_with_replicas_not_ready(
                ...
                table_status) {
                    ...
                    return table_status.map(table_config_id, (status, config) => ({
                        id: status('id'),
                        name: status('name'),
                        db: status('db'),
                        ...
                        shards: status('shards').default([]).map(
                            ...
                            (shard, pos, conf_shard) => ({
                                ...
                                replicas: shard('replicas')
                                    .filter(replica => r.expr(['ready',
                                    'looking_for_primary_replica',
                                    'offloading_data'])
                                    .contains(replica('state')).not()).map(conf_shard('replicas'), (replica, replica_id) => ({
                                    replica_id,
            ...
            num_primaries(table_config_id) {
                ...
                return table_config_id('shards').default([])
                    .map(x => x.count()).sum();
            },
            ...
            num_connected_primaries(table_status) {
                ...
                return table_status.map(table => table('shards').default([])('primary_replicas').count(arr => arr.isEmpty().not())).sum();
            },
            ...
            num_replicas(table_config_id) {
                ...
                return table_config_id('shards').default([])
                    .map(shards => shards.map(shard => shard("replicas").count()).sum()).sum();
            },
            ...
            num_connected_replicas(table_status) {
                ...
                return table_status('shards').default([])
                    .map(shards => shards('replicas').map(replica => replica('state').count(replica => r.expr(['ready', 'looking_for_primary_replica']).contains(replica))).sum()).sum();
            },
            ...
            disconnected_servers(server_status) {
                ...
                return server_status.filter(server => server("status").ne("connected")).map(server => ({
            ...
            if (error != null) {
                ...
                if (this.state === 'fail') {
                    ...
                    return query.private_run(connection, (err, result) => {
                        if (((result != null ? result.value : undefined) != null) && ((result != null ? result.profile : undefined) != null)) {
                            result = result.value;
                        }
                        if (typeof (result != null ? result.toArray : undefined) === 'function') {
                                    ...
                                    return query.private_run(connection, (err, result) => {
                                        ...
                                        if (typeof (result != null ? result.toArray : undefined) === 'function') {
                                            return result.toArray((err, result) => {
                                                ...
                                                if (((err != null ? err.msg : undefined) === "This HTTP connection is not open.") 
                                                        || ((err != null ? err.message : undefined) === "This HTTP connection is not open")) {
    ...
    stop_timer(timer) {
        clearTimeout(this.timers[timer] != null ? this.timers[timer].timeout : undefined);
        __guard__(this.timers[timer] != null ? this.timers[timer].connection : undefined, x => x.close({noreplyWait: false}));
