...
class TableContainer extends Backbone.View {
        ...
        return r.do(
            ...
            (server_config, table_status, table_config) => r.branch(
                ...
                table_status.merge({
                    ...
                    num_default_servers: server_config.filter(server => server('tags').contains('default')).count(),
                    max_num_shards: r.expr([64,
                        server_config.filter((server => server('tags').contains('default'))).count()]).min(),
                    num_primary_replicas:
                        table_status("shards").count(
                            row => row('primary_replicas').isEmpty().not())
                            .default(0),
                    num_replicas: table_status("shards").default([]).map(shard => shard('replicas').count()).sum(),
                    num_available_replicas: table_status("shards").concatMap(
                        shard => shard('replicas').filter({state: "ready"}))
                            .count().default(0),
    ...
    failable_index_query() {
        ...
        return r.do(
            r.db(system_db).table('table_config').get(this.id),
            table_config => // add an id for backbone
        ...
        return r.do(
            ...
            (table_status, table_config, server_name_to_id) => table_status.merge({
                ...
                total_keys: r.db(table_status('db'))
                    ...
                    .info()('doc_count_estimates')
                    .sum(),
                shard_assignments: table_status.merge(table_status => table_status('shards').default([]).map(
    ...
    handle_failable_index_response(error, result) {
        ...
        if (this.table_view != null) {
            this.table_view.set_indexes(this.indexes);
        }
    ...
    handle_failable_misc_response(error, result) {
        ...
        if (this.distribution != null) {
            ...
            this.distribution = new models.Distribution(_.map(
                result.distribution, shard => new models.Shard(shard))
            );
            if (this.table_view != null) {
                this.table_view.set_distribution(this.distribution);
            }
        }

        if (this.shards_assignments != null) {
            this.shards_assignments.set(_.map(result.shard_assignments, shard => new models.ShardAssignment(shard))
            );
        } else {
            ...
            if (this.table_view != null) {
                this.table_view.set_assignments(this.shards_assignments);
            }
        }
    ...
    render() {
        if (this.error != null) {
            this.$el.html(this.template.error({
                error: (this.error != null ? this.error.message : undefined),
                url: '#tables/'+this.id
            })
            );
        } else {
...
class ReconfigurePanel extends Backbone.View {
    ...
    fetch_progress() {
        const query = r.db(system_db).table('table_status')
            .get(this.model.get('id'))('shards').default([])('replicas')
            .concatMap(x => x)
            .do(replicas => ({
    ...
    render() {
        ...
        this.$('.backfill-progress').html(this.progress_bar.render(
            this.model.get('num_available_replicas'),
            this.model.get('num_replicas'),
            {got_response: true}
        ).$el
...
class Profile extends Backbone.View {
    ...
    render() {
        ...
        if ((obj.status != null ? obj.status.ready_for_writes : undefined) && !(obj != null ? obj.status.all_replicas_ready : undefined)) {
...
class SecondaryIndexesView extends Backbone.View {
    ...
    static initClass() {
        ...
        this.prototype.alert_message_template = require('../../handlebars/secondary_indexes-alert_msg.hbs');
    ...
    initialize(data) {
        ...
        this.render();
        if (this.collection != null) { return this.hook(); }
    }
    ...
    fetch_progress() {
        ...
        for (index of Array.from(this.collection.models)) {
        ...
        if (build_in_progress && (this.model.get('db') != null)) {
            const query = r.db(this.model.get('db')).table(this.model.get('name')).indexStatus()
                .pluck('index', 'ready', 'progress')
                .merge( index => ({
                    id: index("index"),
                    db: this.model.get("db"),
    ...
    hook() {
        this.collection.each(index => {
            const view = new SecondaryIndexView({
                model: index,
                container: this
            });
        ...
        this.listenTo(this.collection, 'add', index => {
            ...
            if (this.indexes_view.length === 0) {
                ...
                for (let position = 0; position < this.indexes_view.length; position++) {
                    const view = this.indexes_view[position];
    ...
    create_index() {
        ...
        return (index_name => {
            const query = r.db(this.model.get('db')).table(this.model.get('name')).indexCreate(index_name);
            return driver.run_once(query, (error, result) => {
...
class SecondaryIndexView extends Backbone.View {
    ...
    render_progress_bar() {
        ...
        if (this.progress_bar != null) {
            if (this.model.get('ready') === true) {
                ...
                return this.progress_bar.render(progress, 1, {
                    got_response: true,
                    check: true
                }
                    , () => this.render());
            ...
            return this.$('.progress_li').html(this.progress_bar.render(0, Infinity, {new_value: true, check: true}).$el);
        }
    }
    ...
    delete_index() {
        ...
        const query = r.db(this.model.get('db')).table(this.model.get('table')).indexDrop(this.model.get('index'));
        return driver.run_once(query, (error, result) => {
