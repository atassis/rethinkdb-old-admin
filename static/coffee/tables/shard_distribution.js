...
class ShardDistribution extends Backbone.View {
    ...
    render_data_distribution() {
        let bar_width, left, left1, margin_bar;
        $('.tooltip').remove();
        ...
        const max_keys = (left = d3.max(this.collection.models, shard => shard.get('num_keys'))) != null ? left : 100;
        const min_keys = (left1 = d3.min(this.collection.models, shard => shard.get('num_keys'))) != null ? left1 : 0;
        ...
        const width_of_all_bars = (bar_width*this.collection.length) + (margin_bar*(this.collection.length-1));
        ...
        // Y scale
        const y = d3.scale.linear().domain([0, max_keys*1.05]).range([0, svg_height-(margin_height*2.5)]);
        ...
        // Add rectangle
        const bars = bars_container.selectAll('rect').data(this.collection.map((shard, index) => ({
            index,
        ...
        bars.enter().append('rect')
            ...
            .attr('y', d => svg_height-y(d.num_keys)-margin_height-1) //-1 not to overlap with axe
            .attr('width', bar_width)
            .attr( 'height', d => y(d.num_keys))
            .attr( 'title', d => `Shard ${d.index + 1}<br />~${d.num_keys} keys`);
        ...
        const legends = extra_container.selectAll('.legend')
            .data(axe_legend);
