...
class DatabasesContainer extends Backbone.View {
    ...
    query_callback(error, result) {
        this.loading = false;
        if (error != null) {
            if ((this.error != null ? this.error.msg : undefined) !== error.msg) {
                this.error = error;
            ...
            return this.databases.set(_.map(_.sortBy(result, db => db.name), database => new models.Database(database)), {merge: true});
        }
    }
    ...
    initialize() {
        ...
        this.query = r.db(system_db).table('db_config').filter( db => db("name").ne(system_db)).map(db => ({
            name: db("name"),
            id: db("id"),
            ...
            tables: r.db(system_db).table('table_status').orderBy(table => table("name"))
                ...
                replicas_ready: table('shards').default([]).map(shard => shard('replicas').filter(replica => replica('state').eq('ready')).count()).sum(),
                status: table('status'),
                id: table('id')
            }))
        }));
    ...
    initialize(data) {
        ...
        this.listenTo(this.collection, 'add', database => {
            ...
            if (this.databases_view.length === 0) {
                ...
                let added = false;
                for (let position = 0; position < this.databases_view.length; position++) {
                    const view = this.databases_view[position];
...
class DatabaseView extends Backbone.View {
    ...
    initialize() {
        ...
        this.listenTo(this.collection, 'add', table => {
            ...
            if (this.tables_views.length === 0) {
                ...
                let added = false;
                for (let position = 0; position < this.tables_views.length; position++) {
                    const view = this.tables_views[position];
        ...
        return this.listenTo(this.collection, 'remove', table => {
            ...
            if (this.collection.length === 0) {
                return this.$('.tables_container').html(this.template.empty({
                    element: "table",
                    container: "database"
                })
                );
    ...
    update_collection() {
        return this.collection.set(_.map(this.model.get("tables"), table => new models.Table(table)), {merge: true});
    }
...
class TableView extends Backbone.View {
    constructor(...args) {
        this.initialize = this.initialize.bind(this);
        this.explore_table = this.explore_table.bind(this);
        this.render = this.render.bind(this);
        this.remove = this.remove.bind(this);
        super(...args);
    }
    ...
    explore_table() {
        window.localStorage.current_query = JSON.stringify(`r.db('${this.model.get('db')}').table('${this.model.get('name')}')`);
        return app.main.router.navigate("#dataexplorer", {trigger: true});
    }
