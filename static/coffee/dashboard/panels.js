...
class ServersModel extends Backbone.Model {
    ...
    static query(server_status, 
             table_config) {
        if (server_status == null) { server_status = systable('server_status'); }
        if (table_config == null) { table_config = systable('table_config'); }
        return r.do(
            ...
            (connected_servers, assigned_servers) => ({
                ...
                unknown_missing:
                    table_config.filter(row => row.hasFields('shards').not())('name').isEmpty().not()
            }));
    }
}
...
class ServersView extends vdom.VirtualDomView {
    render_vdom() {
        ...
        const servers_missing =
            unknown_missing && (num_servers_missing > 0) ?
                `${num_servers_missing}+`
            : unknown_missing && (num_servers_missing <= 0) ?
                "1+"
            :
...
class TablesModel extends Backbone.Model {
    ...
    static query(table_status) {
        if (table_status == null) { table_status = systable('table_status'); }
        return {
...
class IndexesModel extends Backbone.Model {
    static initClass() {
        this.query = (table_config, 
                 table_status, 
                 jobs, 
                 current_issues) => {
            if (table_config == null) { table_config = systable('table_config'); }
            if (table_status == null) { table_status = systable('table_status'); }
            if (jobs == null) { jobs = systable('jobs'); }
            if (current_issues == null) { current_issues = systable('current_issues'); }
            return {
                num_sindexes:
                    table_config.sum(row => row('indexes').count()),
...
class ResourcesModel extends Backbone.Model {
    static initClass() {
        this.query = (stats, 
                 table_status) => {
            if (stats == null) { stats = systable('stats'); }
            if (table_status == null) { table_status = systable('server_status'); }
            return {
                cache_used:
                ...
                disk_used:
                    stats.filter(row => row('id').contains('table_server'))(
                        ...
                        .map(data => data('data_bytes').add( 
                    data('garbage_bytes')).add( 
                    data('metadata_bytes')).add( 
                    data('preallocated_bytes'))).sum()
            };
        };
        this.prototype.defaults = {
            cache_used: 0,
...
var render_panel = (panel_name, problems_exist, elements) => {
    const problems_class = problems_exist ?
        "problems-detected"
    :
