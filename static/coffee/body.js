...
class MainContainer extends Backbone.View {
    ...
    fetch_data(me) {
        const query = r.expr({
            ...
            num_available_tables: r.db(system_db).table('table_status')('status').filter( status => status("all_replicas_ready")).count(),
            me: me.proxy ? "<proxy node>" : me.name
        });
...
class OptionsView extends Backbone.View {
    ...
    render() {
        this.$el.html(this.template({
            check_update: ((window.localStorage != null ? window.localStorage.check_updates : undefined) != null) ? JSON.parse(window.localStorage.check_updates) : true,
            version: VERSION
        })
        );
        return this;
...
class AlertUpdates extends Backbone.View {
    ...
    check() {
        ...
        return $.getJSON(`https://update.rethinkdb.com/update_for/${VERSION}?callback=?`, this.render_updates);
    }
    ...
    render_updates(data) {
        if (data.status === 'need_update') {
            ...
            if ((!ignored_version) || (this.compare_version(ignored_version, data.last_version) < 0)) {
                this.next_version = data.last_version; // Save it so users can ignore the update
    ...
    compare_version(v1, v2) {
        ...
        const v1_array = [];
        for (value of Array.from(v1_array_str)) {
...
class IsDisconnected extends Backbone.View {
    ...
    initialize() {
        this.render();
        return setInterval(() => driver.run_once(r.expr(1))
        , 2000);
    }
