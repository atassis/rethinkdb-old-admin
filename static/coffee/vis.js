...
const num_formatter = function(i) {
    let res;
    if (isNaN(i)) {
        return 'N/A';
    }
...
class OpsPlot extends Backbone.View {
    ...
    make_metric(name) {
        ...
        this.context.on('focus', i => {
            return d3.selectAll('.value').style('right', i === null ? null : (this.context.size() - i) + 'px');
        });
    ...
    initialize(_stats_fn, options) {
        if (options != null) {
            ...
            if (options.height != null) { this.HEIGHT_IN_PIXELS =  options.height; }
            if (options.height_in_units != null) { this.HEIGHT_IN_UNITS =   options.height_in_units; }
            if (options.width != null) { this.WIDTH_IN_PIXELS =   options.width; }
            if (options.seconds != null) { this.WIDTH_IN_SECONDS =  options.seconds; }
            if (options.haxis != null) {
                if (options.haxis.seconds_per_tick != null) { this.HAXIS_TICK_INTERVAL_IN_SECS =   options.haxis.seconds_per_tick; }
                if (options.haxis.ticks_per_label != null) { this.HAXIS_MINOR_SUBDIVISION_COUNT = options.haxis.ticks_per_label; }
            }
            if (options.vaxis != null) {
                if (options.vaxis.num_ticks != null) { this.VAXIS_TICK_SUBDIVISION_COUNT =  options.vaxis.num_ticks; }
                if (options.vaxis.ticks_per_label != null) { this.VAXIS_MINOR_SUBDIVISION_COUNT = options.vaxis.ticks_per_label; }
            }
    ...
    render() {
        ...
        // Set up the plot
        this.sensible_plot = this.context.sensible()
        ...
        d3.select(this.$('.plot')[0]).call(div => {
            ...
            // Chart itself
            this.selection = div.append('div')
                .attr('class', 'chart');
            this.selection.call(this.sensible_plot);
            // Horizontal axis
            div.append('div')
                .attr('class', 'haxis')
                .call(this.context.axis()
                    .orient('bottom')
            ...
            div.append('div')
                .attr('class', 'vaxis')
                .call(this.context.axis(this.HEIGHT_IN_PIXELS, [this.read_stats, this.write_stats], this.sensible_plot.scale())
                    .orient('left')
...
class SizeBoundedCache {
    ...
    push(stats) {
        ...
        if (this.values.length < this.ndp) {
            for (let i = this.values.length, end = this.ndp-1, asc = this.values.length <= end; asc ? i < end : i > end; asc ? i++ : i--) {
                this.values.push(value);
            }
        }
...
class InterpolatingCache {
    constructor(num_data_points, num_interpolation_points, get_data_fn) {
        ...
        this.values = [];
        for (let i = 0, end = this.ndp-1, asc = 0 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
            this.values.push(0);
        }
    ...
    push_data() {
        if (this.last_date === null) { // First time we call @push_data
        ...
        for (let i = 1, end = missing_steps, asc = 1 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
            var value_to_push;
