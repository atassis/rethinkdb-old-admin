...
class VirtualDomView extends Backbone.View {
    ...
    initialize(options) {
        this.submodels = (options != null ? options.submodels : undefined) || {};
        this.timers = (options != null ? options.timers : undefined) || {};
    ...
    remove() {
        for (let timer = 0; timer < this.timers.length; timer++) {
            const frequency = this.timers[timer];
...
class BackboneViewWidget {
    ...
    destroy(domNode) {
        ...
        return (this.view != null ? this.view.remove() : undefined);
    }
}
