    ...
    static query(server_configs, server_statuses, table_status) {
        return {
            servers: server_configs.map(server_statuses, (sconfig, sstatus) => ({
                ...
                primary_count: table_status.map(table => table('shards').default([]).count(shard => shard('primary_replicas').contains(sconfig('name')))).sum(),

                secondary_count:
                    table_status.map(table => table('shards').default([]).count(shard => shard('replicas')('server').contains(sconfig('name')).and(
                        shard('primary_replicas').contains(sconfig('name')).not()))).sum()
            })).coerceTo('array')
        };
    }
}
...
class View extends Backbone.View {
    ...
    fetch() {
        ...
        const query = r.expr(args).do(a => Model.query(a('server_config'), a('server_status'), a('table_status')));
    ...
    render_vdom() {
        return h("div.servers-container", [
            ...
            h("table.servers-list.section",
                this.model.get('servers').map(render_server))
        ]);
    }
}

var render_server = server => h("tr.server-container", [
    h("td.name", [
