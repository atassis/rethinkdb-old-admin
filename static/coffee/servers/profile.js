...
class View extends Backbone.View {
    ...
    render_vdom(model) {
        const version = model.version != null ? model.version.split(' ')[1].split('-')[0] : undefined;
        const approx_cache_size = util.format_bytes(model.cache_size);
        return h("div.server-info-view",
            h("div.summary", [
                render_profile_row("hostname", "hostname", model.hostname || "N/A"),
                (version != null) ? render_profile_row("version", "version", version) : undefined,
                (model.time_started != null) ? render_profile_row(
                    "uptime",
                    "uptime",
                    moment(model.time_started).fromNow(true),
                    model.time_started.toString()) : undefined,
                (model.cache_size != null) ? render_profile_row(
                    "cache-size",
                    "cache size",
                    `${approx_cache_size}`) : undefined,
                (model.tags != null) ? render_tag_row(model.tags) : undefined
            ]));
    }
}

var render_profile_row = (className, name, value, title=null) => h(`div.profile-row.${className}`, [
    h(`p.${className}`, [
        h("span.big", {title}, value),
        ` ${name}`
    ])
]);

var render_tag_row = tags => h("div.profile-row.tags", [
    tags.length > 0 ? h("p", [
