...
class Model extends Backbone.Model {
    ...
    static query(table_config, table_status, server_name) {
        return {
            tables: table_config.filter(config => config('shards')('replicas').concatMap(x => x).contains(server_name)).map(function(config) {
...
var render_table = table => h("li.parent", [
...
var render_shard = shard => h("li.child", [
