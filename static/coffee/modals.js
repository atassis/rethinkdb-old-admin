...
class AddDatabaseModal extends ui_modals.AbstractModal {
    ...
    on_submit() {
        ...
        if (error === null) {
            return driver.run_once(r.db(system_db).table("db_config").insert({name: this.formdata.name}), (err, result) => {
                if (err) {
                    ...
                    if (result.inserted === 1) {
                        ...
                        return this.on_error(new Error(result.first_error || "Unknown error"));
                    }
                }
            });
        } else {
    ...
    on_submit() {
        ...
        return driver.run_once(r.dbDrop(this.database_to_delete.get('name')), (err, result) => {
            if (err) {
                ...
                if ((result != null ? result.dbs_dropped : undefined) === 1) {
                    return this.on_success();
                } else {
        ...
        if (input_error === true) {
            ...
            return driver.run_once(query, (err, result) => {
                if (err) {
                    ...
                    if ((result != null ? result.errors : undefined) === 0) {
                        return this.on_success();
                    } else {
...
class RemoveTableModal extends ui_modals.AbstractModal {
    ...
    on_submit() {
        ...
        return driver.run_once(query, (err, result) => {
            if (err) {
                ...
                if ((this.collection != null) && ((result != null ? result.changes : undefined) != null)) {
                ...
                if ((result != null ? result.deleted : undefined) === this.tables_to_delete.length) {
                    return this.on_success();
                } else {
    ...
    on_success(response) {
        ...
        for (let index = 0; index < this.tables_to_delete.length; index++) {
            const table = this.tables_to_delete[index];
...
class ReconfigureModal extends ui_modals.AbstractModal {
    ...
    render() {
        super.render($.extend(this.model.toJSON(), {
            modal_title: "Sharding and replication for " +
                `${this.model.get('db')}.${this.model.get('name')}`,
            ...
            return (() => {
                ...
                for (let error of Array.from(errors)) {
                    ...
                    if (error === 'server-error') {
                        result.push(this.$('.alert.error .server-msg').append(
    ...
    on_submit() {
        ...
        return driver.run_once(query, (error, result) => {
            ...
            } else if (result.first_error != null) {
                return this.model.set({server_error: result.first_error});
            } else {
                this.reset_buttons();
                this.remove();
    ...
    fetch_dryrun() {
        ...
        const query = r.db(this.model.get('db'))
            ...
            .do(diff => r.object(r.args(diff.keys().map(key => [key, diff(key)('shards')]).concatMap(id)))).do(doc => doc.merge({
        ...
        return driver.run_once(query, (error, result) => {
            if (error != null) {
                ...
                return this.model.set({
                    shards: this.fixup_shards(result.old_val,
                        result.new_val,
                        result.lookups)
                });
            }
    ...
    shard_sort(a, b) {
        if (a.role === 'primary') {
            return -1;
        } else if ([b.role, b.old_role].includes('primary')) {
            return +1;
    ...
    fixup_shards(old_shards, new_shards, lookups) {
        ...
        let new_shard, old_role, replica, shard_diff;
        const shard_diffs = [];
        // first handle shards that are in old (and possibly in new)
        for (let i = 0; i < old_shards.length; i++) {
            var shard_deleted;
