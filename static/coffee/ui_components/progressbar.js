...
class OperationProgressBar extends Backbone.View {
    ...
    render(current_value, max_value, additional_info, cb) {
        ...
        if ((this.stage === 'none') && (additional_info.new_value != null)) {
            this.stage = 'starting';
            if (this.timeout != null) { // If there is a timeout, we have to clear it, we are not done yet
                clearTimeout(this.timeout);
                this.timeout = null;
            }
        }
        ...
        if (this.stage === 'finished') {
            ...
            if ((this.timeout == null)) { // If there is already a timeout, no need to add another one
